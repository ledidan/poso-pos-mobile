...
// Lib
import InfoSanitizer from "../../Functions/InfoSanitizer";
import FoodMenuFuncs from "../../Functions/FoodMenuFuncs";
...
const AddTable = ({ tableInfo, shopID }) =>
  new Promise((resolve, reject) => {
    if (!tableInfo || !shopID)
      return reject("(AddTable) Parameters are not sufficient.");
    axios
      .post(API_PATHS.ADD_TABLE, { tableInfo, shopID })
      .then(resolve)
      .catch(reject);
  });
const UpdateTable = ({ tableInfo, tableID, shopID }) =>
  new Promise((resolve, reject) => {
    if (!tableInfo || !shopID)
      return reject("(UpdateTable) Parameters are not sufficient.");
    axios
      .post(API_PATHS.UPDATE_TABLE, { tableInfo, shopID, tableID })
      .then(resolve)
      .catch(reject);
  });

const DeleteTable = ({ tableID, shopID }) =>
  new Promise((resolve, reject) => {
    if (!tableID || !shopID)
      return reject("(DeleteTable) Parameters are not sufficient.");
    axios
      .post(API_PATHS.DELETE_TABLE, { shopID, tableID })
      .then(resolve)
      .catch(reject);
  });
...
const CreateNewItem = ({ itemInfo, shopID }) =>
  new Promise((resolve, reject) => {
    if (!shopID || !itemInfo || Object.keys(itemInfo).length === 0)
      return reject("(CreateNewItem) Parameters are not sufficient.");
    const sanitizedItemInfo =
      FoodMenuFuncs.SanitizeItemInfoBeforeSubmit(itemInfo);
    const params = { itemInfo: sanitizedItemInfo, shopID };
    axios
      .post(API_PATHS.CREATE_NEW_ITEM, params)
      .then(({ data }) => resolve({ newItemID: data, sanitizedItemInfo }))
      .catch(reject);
  });
...
const SaveCheckInInfo = (params) =>
  new Promise((resolve, reject) => {
    const { guest_checkin_info, shop_id } = params;
    if (!Object.keys(guest_checkin_info).length === 0 || !shop_id)
...
const SaveSpecialHours = (params) =>
  new Promise((resolve, reject) => {
    const { shop_id = "" } = params;
    if (!shop_id)
      return reject("(SaveSpecialHours) Parameters are not sufficient.");
    axios
      .post(API_PATHS.SAVE_SPECIAL_HOURS, params)
      .then(() => resolve({ success: true }))
      .catch(() => resolve({ success: false }));
  });
...
// ---------------------------------------------------------------
// Send export file excel to mail
const SendExportPayoutToMail = ({ displayed_data, email, subject, body }) =>
  new Promise((resolve, reject) => {
    if (!displayed_data || !email || !subject || !body)
      return reject("(SendExportPayoutToMail) Parameters are not sufficient.");
    axios
      .post(API_PATHS.SENT_EXPORT_PAYOUT_TO_MAIL, {
        displayed_data,
        email,
        subject,
        body,
...
const ChangeTableStatus = (params) =>
  new Promise((resolve, reject) => {
    const { tableInfo, shopID } = params;
    if (!tableInfo || !shopID)
      return reject("(ChangeTableStatus) Parameters are not sufficient.");
    axios
      .post(API_PATHS.CHANGE_TABLE_STATUS, params)
      .then(() => resolve({ success: true }))
      .catch(() => resolve({ success: false }));
  });
