...
// ---------------------------------------------------------------
// Get the information of guest check-in mode

const GetGuestCheckinInfo = (params = {}) =>
  new Promise((resolve, reject) => {
    if (!params.shop_id)
      return reject("(GetGuestCheckinInfo) Parameters are not sufficient.");
    axios
      .get(API_PATHS.GET_GUEST_CHECKIN_INFO, { params })
      .then(({ data }) => resolve({ guest_checkin_info: data }))
      .catch(reject);
  });

// ---------------------------------------------------------------
// Get the reviews for a shop

const GetGuestReviews = (params) =>
  new Promise((resolve, reject) => {
    if (!params.shop_id)
      return reject("(GetGuestReviews) Parameters are not sufficient.");
    axios
      .get(API_PATHS.GET_GUEST_REVIEWS, { params })
      .then(({ data }) => resolve({ guest_reviews: data }))
      .catch(reject);
  });
...
// ---------------------------------------------------------------
// Get all payouts for the connected account
const GetPayoutsForConnectedAccountByCreated = (params) =>
  new Promise((resolve, reject) => {
    if (!params.stripe_account_id)
      return reject(
        "(GetPayoutsForConnectedAccountByCreated) Parameters are not sufficient."
      );
    axios
      .get(API_PATHS.GET_PAYOUTS_FOR_CONNECTED_ACCOUNT_BY_CREATED, { params })
      .then(({ data = {} }) => resolve({ payouts_info: data }))
      .catch(reject);
  });
...
const GetTable = ({ shopID }) =>
  new Promise((resolve, reject) => {
    if (!shopID) return reject("(GetTable) Parameters are not sufficient.");
    axios
      .get(GET_TABLE, { params: { shopID } })
      .then(({ data }) => {
        resolve({
          tables: typeof data === "string" ? JSON.parse(data) : data,
        });
      })
      .catch(reject);
  });
...
const GetUniqueCustomerPayment = ({ payment_id }) =>
  new Promise((resolve, reject) => {
    if (!payment_id)
      return reject(
        "(GetUniqueCustomerPayment) Parameters are not sufficient."
      );
    axios
      .get(API_PATHS.GET_PAYMENT_INFO, { params: { payment_id } })
      .then(({ data }) => resolve(data))
      .catch(() => resolve({ success: false }));
  });

// ---------------------------------------------------------------
// Export Menu Poso To PDF

const ExportMenuToPDF = ({ shopID, email }) =>
  new Promise((resolve, reject) => {
    if (!shopID)
      return reject("(ExportMenuToPDF) Parameters are not sufficient.");
    axios
      .get(EXPORT_MENU_TO_PDF, {
        params: { shopID, email },
        responseType: "blob",
      })
      .then(({ data }) => resolve(data))
...
// ---------------------------------------------------------------
// Export Menu Poso To Excel

const ExportMenuToExcel = ({ shopID, email }) =>
  new Promise((resolve, reject) => {
    if (!shopID)
      return reject("(ExportMenuToExcel) Parameters are not sufficient.");
    axios
      .get(EXPORT_MENU_TO_EXCEL, {
        params: { shopID, email },
        responseType: "blob",
      })
      .then(({ data }) => resolve(data))
...
// ---------------------------------------------------------------
// Get the items by order

const GetItemsByOrder = ({ orderID, shopID }) =>
  new Promise((resolve, reject) => {
    if (!orderID || !shopID)
      return reject("(GetItemsByOrder) Parameters are not sufficient.");
    axios
      .get(API_PATHS.GET_ITEMS_BY_ORDER, { params: { orderID, shopID } })
      .then(({ data }) => resolve(data))
      .catch(reject);
  });
...
export default {
  ...
  GetPayoutsForConnectedAccountByCreated,
  GetSalesReportData,
